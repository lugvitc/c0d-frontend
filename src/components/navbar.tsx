...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-10 flex w-full items-center justify-between p-4 md:p-12 text-white bg-transparent">
      {/* Logo */}
      <Image
      ...
      {isOpen && (
        <nav className="absolute top-full left-0 w-full bg-black text-white flex flex-col items-center space-y-4 py-6 md:hidden">
          ...
          <LinkButton href="/contact" variant="secondary" onClick={toggleMenu}>
            CHALLENGES
          </LinkButton>
          <LinkButton href="/leaderboard" variant="secondary" onClick={toggleMenu}>
            LEADERBOARD
          </LinkButton>
          <LinkButton href="#prizes" variant="secondary" onClick={toggleMenu}>
            PRIZES
          </LinkButton>
          <LinkButton href="/signin" variant="primary" onClick={toggleMenu}>
            SIGNIN
          </LinkButton>
        </nav>
      )}
